...
class Connection(object):
    ...
    def __str__(self):
        ...
        return '(%u-%u) -> (%u-%u) = %f' % (
            ...
            self.downstream_node.layer_index, self.downstream_node.node_index, self.weight)
...
class Network(object):
    def __init__(self, layers):
        ...
        for layer in range(layer_count - 1):
            connections = [Connection(upstream_node=upstream_node, downstream_node=downstream_node)
                           for upstream_node in self.layers[layer].nodes
                           for downstream_node in self.layers[layer + 1].nodes[:-1]]

            for conn in connections:
                self.connections.add_connection(connection=conn)
    ...
    def train(self, labels, data_set, rate, iteration):
        ...
        for i in range(iteration):
            for d in range(len(data_set)):
                self.train_one_sample(label=labels[d], sample=data_set[d], rate=rate)
    ...
    def calc_delta(self, label):
        ...
        for i in range(len(label)):
            output_nodes[i].calc_output_layer_delta(label=label[i])
    ...
    def get_gradient(self, label, sample):
        ...
        :return:
        """
        self.predict(sample=sample)
        self.calc_delta(label=label)
        self.calc_gradient()
