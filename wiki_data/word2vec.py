...
"""
# I/O bound：I/O 密集型，系统CPU效能相对硬盘和内存好很多，此时大部分CPU在等io的读写，CPU Loading不高。
import time
...
flags = tf.flags
flags.DEFINE_string("model", "two", "A type of model.")
FLAGS = flags.FLAGS
...
def main_word2vec_basic():
    ...
    # Step 1： 加载数据
    words = list()
    with open(file='/home/dengkaiting/pycharm_project/data/all_remove_label.txt_split_word_handler_remove_punctuation_handler') as f:
        lines = f.readlines()
        for line in lines:
            content = line.strip()
    ...
    # Set hyperparameters.
    data_size = len(words)
    print(data_size)  # 191665136
    print(words[0:10])  # ['这个', '页面', '现在', '已经', '作废', '相关', '-', '{', '-', ':']
    # exit()
    ...
    print('Most 5 common words (+UNK)',
          count[:5])  # [['_UNK', 15198906], ('的', 9923712), ('年', 3346786), ('在', 2858385), ('是', 2512035)]
    print('Sample data', data[:10], [reverse_dictionary[i] for i in data[:10]])
    # [142, 817, 356, 194, 27710, 378, 13, 176, 13, 47] ['这个', '页面', '现在', '已经', '作废', '相关', '-', '{', '-', ':']
    ...
    while step < num_steps:
        ...
        if step % (print_freq * 5) == 0:
            # 在运行TensorFlow计算过程中，可以使用eval()获得其中间值，类型为numpy.ndarray
